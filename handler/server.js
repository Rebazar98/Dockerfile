...
app.post('/import', upload.any(), async (req, res) => {
  ...
  try {
    ...
    const filePart = (req.files || []).find(f => f.fieldname === 'data') ||
    ...
    // Detectar si trae SRS
    const checkCmd = `ogrinfo -ro -so -al "${srcForOgr}" ${layerArg}`;
    let hasSRS = false;
    try {
      const info = await new Promise((ok) => exec(checkCmd, { maxBuffer: 16 * 1024 * 1024 }, (_, out='') => ok(String(out))));
    ...
    exec(cmd, { maxBuffer: 64 * 1024 * 1024, timeout: 180000 }, (err, stdout, stderr) => {
